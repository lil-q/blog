...
首先写出矩形类，定义长和宽，并写出长宽的 *setter* 方法和一个获取面积的方法：
...
长方形类和正方形类作为子类继承抽象矩形类。对于长方形，定义两个 *final*  修饰的字段表示长宽，对于正方形，定义一个 *final* 修饰的字段表示边长，这是为了保证特征属性的不可变。分别重写父类的两个抽象方法和接口的一个抽象方法：
