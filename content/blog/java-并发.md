...
1. 在工作内存中，每次使用 Ⅴ 前都必须先**从主内存刷新最新的值**，用于保证能看见其他线程对变量所做的修改。

   即 use 动作 与 load、read 动作相关联的，必须连续且一起出现。

2. 在工作内存中，每次修改Ⅴ后都必须**立刻同步回主内存中**，用于保证其他线程可以看到自己对变量V所做的修改。

   即 assign 动作与 store、write 动作相关联的，必须连续且一起出现。
